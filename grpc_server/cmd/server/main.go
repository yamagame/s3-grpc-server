...
func main() {
	...
	s := grpc.NewServer()
	aws.RegisterStorageServer(s, storageService.NewStorageServer(storage.GetClient(mode)))
	aws.RegisterRepositoryServer(s, repositoryService.NewRepositoryServer(repository.GormDB()))
